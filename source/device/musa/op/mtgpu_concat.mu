...
void concat_gpu_kernel(struct graph* ir_graph, struct node* ir_node, dict_uint2voidx  gpu_addr_map)
{
    ...
    for (int i = 0; i < ir_node->input_num; i++)
    {
        ...
        {
            musaMemcpy((float*)gpu_addr_map[output_tensor->index] + sum_size + p * elem_num_axis_sum, (float*)gpu_addr_map[input_tensor->index] + p * elem_num_axis,
                       elem_num_axis * sizeof(float), musaMemcpyDeviceToDevice);
        }
...
void MUSAEngine::AddConcatNode(struct graph* ir_graph, struct node* ir_node)
{
